 ...
 * due to failures in later steps or child handlers.
 * 
 * ## Two-Phase Rollback Process
 * 
 * **Important insight**: Each original [TransactionalStep] is split into **two steps** during rollback:
 * 
 ...
 * 
 * The rollback continuation processes these sub-steps in sequence, suspending between each
 * to wait for the [EventLoopStrategy][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.EventLoopStrategy]
 * to signal readiness (typically when all child handlers have completed).
 * 
 * Step name generation uses unique prefixes/suffixes to avoid conflicts with user-defined
 * step names, automatically adding '$' characters if collisions are detected.
 * 
 ...
 * 
 * This transforms the saga's linear step sequence into a two-phase rollback execution
 * plan and determines exactly where to resume rollback based on the current state.
 * 
 * ## High-Level Process
 * 
 ...
 * ## Entry Point Logic
 * 
 * **Case 1: NotSuspendedYet** - Rollback during the first step, before anything was
 * actually committed
 ...
 * ## Collision Avoidance
 * 
 * The method automatically detects if user-defined steps conflict with generated rollback
 * step names and appends '$' characters until uniqueness is achieved.
 * 
    ...
    return when (coroutineState.lastSuspendedStep) {
        ...
        is LastSuspendedStep.SuspendedAfter -> {
            ...
            if (cleanLastSuspendedStepName == coroutineState.lastSuspendedStep.stepName) {
                ...
                // Formula: For original step at index N, the "child rollback" sub-step is at index 2*N+1
                // Example:
                // Original steps: ["step A", "step B", "step C"]
                // Original indices: [0, 1, 2]
                // Rollback steps: ["Rollback of step A (rolling back child scopes)", "Rollback of step A",
                //                  "Rollback of step B (rolling back child scopes)", "Rollback of step B", 
                //                  "Rollback of step C (rolling back child scopes)", "Rollback of step C"]
                // Rollback indices: [0, 1, 2, 3, 4, 5]
                //
                // If step C (index 2) failed, we want to start with "Rollback of step C (rolling back child scopes)"
                // Formula: 2 * 2 + 1 = 5, which gives us rollbackSteps[5] = "Rollback of step C (rolling back child scopes)"
                ...
                if (rollbackSteps[nextStepIdx] == rollbackSteps.first()) {
                    // We're on the first (i.e., last in rollback execution order) step - rollback is completing
                    ...
                    // Note: rollbackSteps are in reverse execution order, so we subtract 1 to get "next"
                    RollbackPathContinuation(
                        connection,
                        coroutineState.cooperationContext,
                        coroutineState.scopeIdentifier,
...
/**
 * Transforms the original saga steps into rollback sub-steps for two-phase rollback execution.
 * Each original step becomes two rollback sub-steps that execute in reverse order to ensure
 ...
 *    - Emits rollback messages to all child handlers via [ScopeCapabilities.emitRollbacksForEmissions]
 * 
 ...
 * @param rollingBackSuffix Unique suffix for child rollback steps (e.g., " (rolling back child scopes)")
