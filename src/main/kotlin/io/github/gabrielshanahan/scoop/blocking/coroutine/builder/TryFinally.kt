 ...
 * ## How It Works
 * 
 * The implementation uses [CooperationContext] to track which finally blocks have already
 * been executed. This prevents duplicate executions during complex rollback scenarios.
 * 
 * ## Usage Pattern
 * 
 * ```kotlin
 ...
 * 
 * This element maintains a list of step names for which the finally block has already
 * been executed. The [plus] method ensures that when contexts are combined, the lists
 * of executed finally blocks are merged.
 * 
 ...
 * 
 * This updates the [CooperationContext] to track that the finally block has run,
 * preventing it from running again during rollback scenarios.
 * 
