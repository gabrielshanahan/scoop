 ...
 * [fetching messages][fetch] and [subscribing handlers (sagas) to a topic][subscribe]. Callers
 * are responsible for transaction management (by supplying a suitable [Connection]) and payload
 ...
 *
 * Delegates to [MessageRepository], [StructuredCooperationCapabilities] and [EventLoop] for most of the
 * actual work.
 */
@ApplicationScoped
class PostgresMessageQueue(
    ...
    /**
     * Subscribes a [saga] (see [SagaBuilder][io.github.gabrielshanahan.scoop.blocking.coroutine.builder.SagaBuilder]) to a [topic].
     *
     * This amounts to doing two things:
     * 1) start a periodic process that checks if any saga is ready to be resumed, i.e., its next step
     * is ready to be executed. The technical term for this kind of periodic process is an 'event loop',
     * and a single iteration a 'tick'. See [EventLoop].
     *
     * 2) set up a notification pipeline that will run a 'tick' whenever a message is published to a topic.
     * This is done via Postgres [LISTEN](https://www.postgresql.org/docs/current/sql-listen.html), and
     * is a non-critical part of the implementation - the periodic process above will always run `tick`
     * eventually. The [NOTIFY](https://www.postgresql.org/docs/current/sql-notify.html) is done in a
