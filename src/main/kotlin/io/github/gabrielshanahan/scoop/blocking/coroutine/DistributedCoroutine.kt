...
/**
 * Represents a single step in a distributed saga, which is represented as a [DistributedCoroutine]. You
 * will rarely want to build instances of these objects manually - instead, use
 ...
 *
 * In structured cooperation, when a step emits messages using `scope.launch()`, the handlers
 * of those messages are called "child handlers" (or "child sagas") of this saga. This parent-child
 * relationship is tracked using something called
 * [cooperationLineage][io.github.gabrielshanahan.scoop.shared.coroutine.CooperationScopeIdentifier.cooperationLineage] - basically,
 * a list of UUIDs, which each "child" emission inherits from its parent, appends its own UUID to,
 * and passes on to *its* children.
 *
 * Some key ways this parent-child relationship is used:
 * - **Parent waits for children**: The saga suspends after this step and won't proceed until
 *   ALL child handlers have completed (successfully or via rollback). This is actually
 *   encapsulated by the default instance of [EventLoopStrategy] (see
 ...
 *   [SleepEventLoopStrategy][io.github.gabrielshanahan.scoop.blocking.coroutine.builder.SleepEventLoopStrategy] does.
 * - **Child failures propagate up**: If any child handler fails, those failures bubble up to
 *   this parent step, and into [handleChildFailures]. See [EventLoop] and
 ...
 * - **Rollback propagates down**: If a step needs to roll back, rollback messages are first sent
 *   to all child handlers, and only after they have finished rolling back does this step's
 *   [rollback] method runs. See [RollbackPathContinuation][io.github.gabrielshanahan.scoop.blocking.coroutine.continuation.RollbackPathContinuation].
 *
...
interface TransactionalStep {
     ...
     * 
     * This method runs within a database transaction. Any database operations executed
     * using the [scope]'s [connection][CooperationScope.connection] during this method's
     * execution will be committed atomically when the step finishes executing (or rolled
     * back if an exception happens). This includes emitting messages via [CooperationScope.launch]
     * and [CooperationScope.launchOnGlobalScope].
     * 
     * After this method completes successfully, the saga will suspend and wait for the associated
     * [DistributedCoroutine.eventLoopStrategy] to signal that it should resume - typically, this
     * is implemented as waiting for all handlers of all emitted messages to complete, which is
     * the core mechanism that implements structured cooperation.
     * 
     ...
     * 
     * When a child message handler (child saga) fails, this method is called before the saga
     ...
     *   runs in a separate transaction. That means that any messages emitted during its
     *   execution will be treated as if they were emitted from [invoke].
     * - This applies mainly to rollbacks, where one might expect rollbacks to first happen
     *   for messages emitted from [handleChildFailures], and only then for messages emitted
     *   from [invoke]. However, in the current POC implementation, no guarantees are made
     *   about rollback order, and all ROLLBACK_EMITTED events are emitted concurrently.
     ...
     * - This is the main reason why Scoop doesn't provide a retry abstraction, since this needs
     *   to be accounted for manually
     * 
     * The default implementation re-throws the exception, causing the saga to enter
     * rollback mode.
     *
     * **Future improvement**: A cleaner approach would be to have a separate rollback step for anything
     * emitted from [handleChildFailures], but this would require supporting a dynamic number "semi-steps"
     * between each step (has [handleChildFailures] can run multiple times), which is definitely doable,
     * but out of scope for a rudimentary POC.
     * 
...
/**
 * Represents a distributed saga - a sequence of [TransactionalStep]s that can be suspended and resumed.
 * 
 * ## Why "Coroutine"?
 * 
 * A **coroutine** is a programming concept representing a function that can be suspended and resumed.
 * Unlike regular functions that run from start to finish, coroutines can pause their execution at
 * specific points and continue later from exactly where they left off.
 * 
 ...
 * coroutines suspend within a single JVM process, while DistributedCoroutines suspend across database
 * transactions and distributed message handling.
 *
