...
/**
 * Since the lifetime of a [CooperationScope] is exactly the same as the lifetime
 * of the (delimited) [Continuation], there's no reason not to use the continuation
 * itself as the [CooperationScope]. Continuing the book metaphor from [Continuation],
 * if you think of a delimited continuation as the text of a page, the scope can be
 * thought of as the paper on which it is written. It's two different things, but
 * it makes sense to combine the two and just call the result "a page".
 */
 ...
 * and waits for child handlers to complete (or, more specifically, for the
...
internal sealed interface SuspensionPoint {
     ...
     * 
     * This occurs either when a new saga is triggered by a message but hasn't executed
     * any steps yet, or when a rollback has just started but hasn't rolled back any steps
     * yet. The continuation will execute the first step (or roll back the last successful
     * step) when resumed.
     */
     ...
     * 
     * This represents a point in the execution of the saga when it has finished executing
     * its final step and is waiting for any final child handlers to complete.
     */
 ...
 * @param scopeCapabilities Provides the implementations of capabilities exposed by [CooperationScope] that require interacting with the database
 */
internal abstract class BaseCooperationContinuation(
    ...
    /**
     * Tracks the current step being executed within this continuation. Since suspension
     * points occur "between steps" (after one step finishes, before the next begins), which
     * one is "current" changes during the continuation's execution.
     * 
     * ### Key Decision Logic (see [resumeWith])
     * 
     * **For BetweenSteps suspension points:**
     * - **Child failure** (`LastStepResult.Failure`): Handle failures on the **previous step** 
     *   (the step that emitted the messages that failed) via [TransactionalStep.handleChildFailures]
     * - **Success** (`SuccessfullyInvoked`/`SuccessfullyRolledBack`): Execute the **next step**
     ...
     * 
     * 1. **Message attribution**: All emitted messages are tagged with [currentStep.name][TransactionalStep.name]
     * in the database for structured cooperation lineage tracking
     ...
     * simple boolean condition, this returns SQL that **selects exception records** when
     * giving up conditions are met.
     * 
     * ## How it works
     * 
     ...
     * ## Return format
     * 
     * The SQL should return rows with an `exception` column containing JSONB representations
     * of [CooperationFailure][io.github.gabrielshanahan.scoop.shared.coroutine.structuredcooperation.CooperationFailure] objects. These get processed by
     ...
     * and converted to [CooperationException][io.github.gabrielshanahan.scoop.shared.coroutine.structuredcooperation.CooperationException]s.
     * 
     * ## Common give-up scenarios
     * 
     * - **Timeouts**: Deadline violations in [CooperationContext] return constructed deadline exceptions
     * - **Cancellation**: CANCELLATION_REQUESTED events return the stored cancellation exception
     * - **Manual intervention**: Custom logic can return appropriate failure records
     * 
     * ## Implementation examples
     * 
     * See concrete implementations in:
     * - [HappyPathContinuation.giveUpStrategy]: Uses [BaseEventLoopStrategy.giveUpOnHappyPath][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.BaseEventLoopStrategy.giveUpOnHappyPath]
     * - [RollbackPathContinuation.giveUpStrategy]: Uses [BaseEventLoopStrategy.giveUpOnRollbackPath][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.BaseEventLoopStrategy.giveUpOnRollbackPath]
     * 
     * For SQL examples, see [cancellationRequested] and [deadlineMissed] functions.
     * 
     * @param seen SQL table alias for the SEEN event record being evaluated
     ...
     * 
     * This is the main entry point for continuation resumption and implements the core
     * structured cooperation logic that determines which step should execute based on:
     ...
     * 
     * **BeforeFirstStep**: Always execute the first step (saga just started or rollback just started)
     * 
     ...
     * 
     * @param lastStepResult What happened in the previous execution cycle - either success or failure of child handlers
     * @return [Continuation.ContinuationResult] indicating whether the saga should suspend again or has completed
     * 
     ...
     * resumption, while delegating the **business logic** to [handleFailuresOrResume]. It also checks
     * if the saga should be given up, before and after the step has been invoked. The give-up checks
     * use the [giveUpStrategy] method to determine if this continuation should abandon execution.
     *
     * @param lastStepResult The result from the previous execution cycle
     * @return [Continuation.ContinuationResult.Failure] if execution should fail,
     *         or the result from [handleFailuresOrResume] if execution proceeds normally
     * 
     ...
     *
     * This method implements the **business logic dispatch** for continuation execution.
     * It determines which method on the [currentStep] should be called based on what
     * happened in the previous execution cycle, implementing the structured cooperation
     * execution model.
     *
     * The actual execution flow control is handled by [resume], while this method focuses
     * purely on the dispatch logic.
     *
     * ## Dispatch Logic
     *
     ...
     * - Always results in suspension because failure handling may emit retry messages
     ...
     * 1. **Failure handling happens at the right step**: The step that emitted failing
     *    messages is responsible for handling the failures
     * 2. **Message attribution is correct**: All emitted messages are tagged with the
     *    [currentStep] that was set by [resumeWith]
     * 3. **Execution mode consistency**: Happy path vs rollback path execution is
     *    maintained across suspension/resumption cycles
     *
     * ## Suspension Behavior
     *
     * - **Failure handling**: Always suspends with emitted messages (even if none were emitted)
     * - **Normal execution**: Uses [resume] helper to determine suspension vs completion
     * - **Rollback execution**: Uses [resume] helper to determine suspension vs completion
     *
     * @param lastStepResult What happened in the previous execution cycle
     * @return [Continuation.ContinuationResult.Suspend] with any emitted messages if the saga
     *         should wait for child handlers, or [Continuation.ContinuationResult.Success] if
     *         the saga has completed entirely
     *
     ...
     *
     * @param resumeStep Lambda containing the step execution logic ([TransactionalStep.invoke] or [TransactionalStep.rollback])
     * @return [Continuation.ContinuationResult.Success] if the saga is complete, or 
     *         [Continuation.ContinuationResult.Suspend] if the saga should wait for child handlers
     * 
     * @see SuspensionPoint.AfterLastStep For the completion detection condition
     * @see Continuation.ContinuationResult For the possible return values and their meaning
     */
