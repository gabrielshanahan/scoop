 ...
 *
 * If you're familiar with Kotlin coroutines, you'll find something similar was done
 * in their implementation (see [kotlinx.coroutines.AbstractCoroutine])
 */
 ...
 * 
 * These suspension points correspond to the places when a saga pauses execution
 ...
 * for their execution modes. The way things are currently written, we actually do some logic
 * in this class that should conceptually be part of those two specializations (specifically,
 * what part of [TransactionalStep] we invoke in [handleFailuresOrResume]). This is done
 * deliberately, as moving it to the subclasses actually makes the code more messy - try it, and
 * you'll see.
 *
...
internal abstract class BaseCooperationContinuation(
     ...
     * of [CooperationFailure][io.github.gabrielshanahan.scoop.shared.coroutine.structuredcooperation.CooperationFailure]
     ...
     * @return [Continuation.ContinuationResult] indicating whether the saga should suspend, has completed, or failed
     * 
     ...
     * happened in the previous execution cycle.
     *
     ...
     * - If it doesn't throw, then always results in suspension because failure handling
     * may emit messages of its own
