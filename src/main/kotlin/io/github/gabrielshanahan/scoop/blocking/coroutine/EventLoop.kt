 ...
 * which is run [periodically, or when NOTIFIED by Postgres][io.github.gabrielshanahan.scoop.blocking.messaging.PostgresMessageQueue.subscribe].
 * 
 * ## How It Works
 *
 ...
 * and [EventLoopStrategy.resumeRollbackPath][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.EventLoopStrategy.resumeRollbackPath]).
 * 2. **Building continuations**: Construct a [Continuation] object that represent the saga's current state
 ...
 *
 * Each saga has its own event loop, i.e., a separate LISTEN/NOTIFY, and a periodic
 * process is started for each saga. See
...
class EventLoop(
     ...
     * 
     * When a new message arrives that should trigger a saga, we need to "start" its continuation
     * by marking the message as "SEEN" in the database. This creates the initial execution state
     * that allows the saga to begin running. Similarly, when rollbacks are triggered, we start
     * rollback continuations, which consists of the [rollback][TransactionalStep.rollback] portions
     * of the steps, in reverse order to how they were executed.
     * 
     * The term "continuation" comes from programming language theory, where it represents "the
     * rest of the computation." In Scoop's case, each saga continuation represents the remaining
     * steps that need to execute for that particular message.
     * 
     * This method will process all ready sagas of the given type before returning.
     * If no sagas are ready, it returns immediately.
     * 
     ...
     * [thundering herd problems](https://en.wikipedia.org/wiki/Thundering_herd_problem)
     ...
     * ## Saga State Overview
     * 
     * This method constructs a [CoroutineState] that captures where a saga is in its lifecycle.
     * The state consists of two key dimensions:
     * 
     ...
     * - **SuccessfullyRolledBackLastStep**: This saga successfully completed rolling back a step that 
     *   had previously failed (either the step itself failed, or it was rolled back due to child failures).
     *   This is like "Gucci" but for rollback execution - ready to continue rolling back, which means rolling
     *   back the previous step.
     * - **ChildrenFailedAndSuccessfullyRolledBack**: Child sagas failed, but their rollbacks succeeded
     * - **ChildrenFailedAndFailedToRollBack**: Child sagas failed AND their rollbacks also failed
     * - **ChildrenFailedWhileRollingBackLastStep**: Complex failure during rollback of this saga's step
     * 
     * **Note on the meaning of "Children"**: In structured cooperation, when a saga step emits messages via
     * `scope.launch()`, the handlers of those messages become "child sagas" of this saga. The parent
     * saga waits for all children to complete before proceeding to its next step. If any child fails,
     * that failure propagates back to the parent, potentially triggering rollbacks.
     * 
     * ## State Combinations
     * 
     ...
     * - **(SuspendedAfter, ChildrenFailed...)**: [Handle child failures][TransactionalStep.handleChildFailures] and
     * potentially start rolling back (or failing the rollback if one is in progress)
     * 
    ...
    private fun fetchSomePendingCoroutineState(
        ...
        if (result == null) {
            ...
            // Exceptions from child sagas that successfully rolled back (runs that finished with ROLLED_BACK), if any
            ...
            // Exceptions from child rollbacks that failed (runs that finished with ROLLBACK_FAILED), if any
            val childRollbackFailedExceptions =
                jsonbHelper
                    .fromPGobjectToList<CooperationFailure>(result.childRollbackFailedExceptions)
                    .map(CooperationFailure::toCooperationException)
            
            // The exception that caused THIS saga run to enter rollback mode (from the ROLLING_BACK event
            ...
            // Context handling - two types of context are tracked separately. This only becomes relevant in situations where
            // the current saga actually completed correctly in the past. However, then a rollback happened somewhere down the
            // line, e.g., due to a parent saga triggering it, so we've come back to this saga run to roll it back, and we're
            // just about to execute the first rollback step.
            // In this specific situation, there are actually two places we want to take the context from:
            // 1) the context at the end of our successful run
            // 2) the context of whatever triggered the rollback (either the parent saga or the user manually)

            // Represents 1) from the description above
            // Only populated during the first rollback step of a previously successfully finished saga run
            ...
            // (from last_event.context - the most recent SEEN/SUSPENDED/COMMITTED/ROLLING_BACK event)
            val latestContext =
                ...
                when {
                    ...
                    childRollbackFailedExceptions.isNotEmpty() ->
                        if (rollingBackException == null) {
                            // Children failed, and their rollbacks also failed, but this saga hasn't started rolling back yet
                    ...
                    childRolledBackExceptions.isNotEmpty() ->
                        if (rollingBackException == null) {
                            // Children failed and successfully rolled back, now this saga's step needs to handle it using Step.handleChildFailures,
                            // or fail itself
            ...
            return CoroutineState(
                ...
                // If there are conflicting values for a key in these contexts, we want the context from the
                // rollback trigger (i.e., latestContext) to take precedence, since that comes chronologically
                // later
     ...
     * 
     * - **Step execution is transactional**: If the step throws an exception, all database
     *   changes (including any messages emitted) are rolled back
     ...
     * - **Message emission is part of the transaction**: Any `scope.launch()` calls within
     *   the step are committed as part of the same transaction
     * 
 ...
 * @param lastSuspendedStep The last step that completed, or [LastSuspendedStep.NotSuspendedYet] for new sagas
