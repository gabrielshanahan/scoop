 ...
 * 
 * This module implements sleep/scheduling functionality. Sagas emit messages to a special
 * sleep topic, which is "bundled with" Scoop, and which uses a
 ...
 * 
 * ### Basic Delays
 * `sleepForStep(duration)` creates a saga step that pauses execution for a specified duration.
 * The step simply emits a message to the [SLEEP_TOPIC] with a [SleepUntil] context element:
 * ```kotlin
 ...
 * 3. If counter < runCount, use `launchOnGlobalScope` to launch the same message again,
 * thereby starting a new, independent run of the entire saga
 * 4. Continue executing the rest of the current saga (the new one starts at 1., so sleeping
 * for the specified duration)
 *
 * There is an inherent tradeoff in how periodic tasks can be implemented:
 * * You can emit the "plan next run" message *before* executing the actual logic. This
 * guarantees that a next run is always planned and will always get executed regardless of
 * the result of the previous run. However, it may cause overlapping runs if the saga runs
 * for longer than the periodicity.
 * * You can emit the "plan next run" message *after* executing the actual logic. This
 * guarantees that there are no overlapping runs, however you need to arrange for the
 * message to be emitted even when the saga run finishes with a failure `nd handle the
 * arithmetic calculating when the next run should be scheduled (now + max(periodicity - executionTime, 0)).
 *
 * For simplicity, Scoop implements the former approach, even though it is perfectly
 * possible to implement the latter using [tryFinallyStep] and additional[CooperationContext]
 * elements.
 *
 ...
 * making this functionality available out-of-the-box without
 * additional setup. The unique [topic UUID][SLEEP_TOPIC] prevents accidental collisions with
 * user-defined topics.
 */
 ...
 * 
 * This element is used by [SleepEventLoopStrategy] to determine when
...
/**
 * Helper method for creating [SleepUntil] instances using relative
 * time.
 * 
 ...
 * 
 * This function creates a single saga step that emits a sleep message to the [SLEEP_TOPIC].
 * The saga will suspend at this step and only resume when the [SleepEventLoopStrategy]
 * determines that the specified time has elapsed.
 * 
 ...
 * 
 * This is a convenience overload of [sleepForStep] that automatically generates
 * a step name using the current step index. Useful for quick prototyping or when
 * step naming isn't critical.
 * 
 * @param duration How long to sleep before resuming
 ...
 * 
 * This enables "execute at specific time" patterns like sending follow-up emails
 * or running maintenance tasks at predetermined times.
 * 
 * ## Usage Example
 * 
 * ```kotlin
 ...
 * 
 * This is a convenience overload of [scheduledStep] that automatically generates
 * a step name using the current step index.
 * 
 ...
 * 
 * Used by [periodic] to track how many times a periodic saga has executed,
 * enabling automatic termination after a specified number of runs.
 */
 ...
 * 
 * Alternative "schedule after" approaches are possible using [tryFinallyStep] and custom
 * context management, but require more complex timing calculations.
 * 
 * ## Usage Example
 * 
 * ```kotlin
 ...
 * 
 * This is a convenience overload of [periodic] that automatically generates
 * a base name using the current step index.
 * 
 * @param runEvery Duration to wait between executions
