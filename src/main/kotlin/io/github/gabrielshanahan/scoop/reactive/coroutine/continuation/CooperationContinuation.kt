...
internal abstract class BaseCooperationContinuation(
     ...
     *    [EventLoop.mark][io.github.gabrielshanahan.scoop.reactive.coroutine.EventLoop.mark])
     * 2. **Rollback events**: ROLLING_BACK/ROLLED_BACK events are attributed to the current step
     *    (see [EventLoop.mark][io.github.gabrielshanahan.scoop.reactive.coroutine.EventLoop.mark])
     ...
     * [MessageEventRepository.fetchGiveUpExceptions][io.github.gabrielshanahan.scoop.reactive.coroutine.structuredcooperation.MessageEventRepository.fetchGiveUpExceptions]
     * and converted to
    ...
    ): Uni<Message> = scopeCapabilities.launch(this, topic, payload, additionalContext)

    override fun launchOnGlobalScope(
        topic: String,
        payload: JsonObject,
        context: CooperationContext?,
    ): Uni<CooperationRoot> = scopeCapabilities.launchOnGlobalScope(this, topic, payload, context)
     ...
     * @return [Uni] of [Continuation.ContinuationResult] indicating whether the saga should
     *   suspend, has completed, or failed
