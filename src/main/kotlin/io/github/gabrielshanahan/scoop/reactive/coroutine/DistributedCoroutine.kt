 ...
 * [SagaBuilder][io.github.gabrielshanahan.scoop.reactive.coroutine.builder.SagaBuilder].
 *
 ...
 *   [SleepEventLoopStrategy][io.github.gabrielshanahan.scoop.reactive.coroutine.builder.SleepEventLoopStrategy]
 *   does.
 ...
 *   [BaseCooperationContinuation.resumeWith][io.github.gabrielshanahan.scoop.reactive.coroutine.continuation.BaseCooperationContinuation.resumeWith].
 ...
 *   method runs. See
 *   [RollbackPathContinuation][io.github.gabrielshanahan.scoop.reactive.coroutine.continuation.RollbackPathContinuation].
 *
 ...
 * ## Reactive Patterns
 *
 * In the reactive implementation, all step methods return `Uni<Unit>` to enable composition with
 * other reactive operations. Error handling follows standard reactive patterns where exceptions are
...
interface TransactionalStep {
     ...
     *   step to resume
     * @return A [Uni] that completes when the step finishes execution
     */
     ...
     * @return A [Uni] that completes when the rollback action finishes
     */
     ...
     * @return A [Uni] that completes normally to continue, or fails to trigger rollback
 ...
 *
 * While this reactive implementation uses Mutiny's `Uni` types (which are built on reactive
 * principles), Scoop's DistributedCoroutines operate at a much higher level than either Kotlin
 * coroutines or reactive streams. Kotlin coroutines suspend within a single JVM process, while
 * DistributedCoroutines suspend across database transactions and distributed message handling.
 *
 * The reactive implementation leverages `Uni<T>` for local asynchronous composition within each
 * step, but the distributed suspend-and-resume behavior is still managed by the database-driven
 * event loop system.
 *
