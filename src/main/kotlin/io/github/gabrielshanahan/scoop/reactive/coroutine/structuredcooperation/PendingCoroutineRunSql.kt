 ...
 *       (see
 *       [EventLoop.fetchSomePendingCoroutineState][io.github.gabrielshanahan.scoop.reactive.coroutine.EventLoop.fetchSomePendingCoroutineState])
 ...
 * in reactive message processing, and distinguishes which "pass" we're currently running.
 */
 ...
 *   of reactive message processing implementation, and noticing that we never specify the `step`.
 *   Contrast that with rollback event handling in the reactive EventLoop, which is what writes
 *   those events when the failure was caused during the run of this saga.
 */
