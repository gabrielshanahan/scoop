 ...
 * ## Reactive Patterns
 *
 * In the reactive implementation, all operations return `Uni<T>` to enable composition with other
 * reactive operations. Error handling follows standard reactive patterns where exceptions are
...
interface CooperationScope {
     ...
     * see
     * [CooperationContinuation][io.github.gabrielshanahan.scoop.reactive.coroutine.continuation.CooperationContinuation]
     * for an explanation why that's the case.
     */
    val continuation: Continuation

    /**
     * The reactive database connection for this saga's current transaction.
     *
     * This connection is used for any database operations within the current step. All operations
     * using this connection will be committed or rolled back atomically with message emissions. In
     * the reactive implementation, this is a Vert.x SqlConnection that supports reactive
     * composition.
     */
     ...
     * @param payload The message payload (JSON object)
     * @param additionalContext Optional context to merge with the current context for child
     *   handlers
     * @return A [Uni] that completes with the emitted message with its assigned ID and scope
     *   information
     */
    fun launch(
        topic: String,
     ...
     * @return A [Uni] that completes with a cooperation root representing the independent scope
     */
     ...
     *
     * @return A [Uni] that completes normally if cancellation is not needed, or fails with an
     *   exception if the saga should be cancelled
     */
