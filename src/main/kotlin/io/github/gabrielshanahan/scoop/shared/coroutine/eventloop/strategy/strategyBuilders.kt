 ...
 * 
 * Checks if any `CANCELLATION_REQUESTED` event exists in the cooperation lineage hierarchy
 * that would affect the current saga. When cancellation is requested, the fragment returns
 * the exception details that should be thrown to abort the saga.
 *
 ...
 * 
 * Happy path deadlines apply during normal saga execution (non-rollback scenarios).
 * The deadline is checked against the most recent SUSPENDED or SEEN event for the saga.
 * 
 ...
 * 
 * Rollback deadlines apply during compensating action execution. The deadline is checked
 * against the most recent SUSPENDED or ROLLING_BACK event for the saga.
 * 
 * @param seen Table alias for the SEEN event being checked
 ...
 * 
 * Absolute deadlines apply to the entire saga lifecycle regardless of whether it's in
 * happy path or rollback execution. Checked against SEEN, SUSPENDED, or ROLLING_BACK events.
 * 
 * @param seen Table alias for the SEEN event being checked
 ...
 * 
 * Generates SQL that checks if a deadline of the specified type has been exceeded by comparing
 * the deadline timestamp stored in the saga's CooperationContext against the current time.
 * Returns a properly formatted CooperationFailure exception if the deadline was missed.
 * 
...
fun deadlineMissed(seen: String, deadlineType: String, eventTypes: List<String>): String {
    // Both of the following need to match the names of the classes, e.g. HappyPathDeadline and HappyPathDeadlineKey
 ...
 * 
 * This is a major part of structured cooperation and is at the heart of the (dummy) approach
 * Scoop uses to solve the
