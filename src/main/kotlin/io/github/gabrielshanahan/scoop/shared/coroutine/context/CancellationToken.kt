 ...
 * implementations to determine when sagas should be cancelled. The most common
 * implementation is deadlines, but the framework is extensible for other cancellation criteria.
 * 
 * ## Distinction from CANCELLATION_REQUESTED
 * 
 * **Important**: CancellationToken represents *automatic* cancellation based on
 * system-defined criteria (timeouts, resource limits, etc.). This is completely
 * separate from `CANCELLATION_REQUESTED` message events, which represent *user-initiated*
 * cancellation (e.g., someone clicks a cancel button). See
 ...
 * - [giveUpOnHappyPath][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.EventLoopStrategy.giveUpOnHappyPath] - checks tokens to cancel normal execution
 * - [giveUpOnRollbackPath][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.EventLoopStrategy.giveUpOnRollbackPath] - checks tokens to cancel rollback execution
 * 
 * The strategy generates SQL queries that examine the saga's CooperationContext for
 * cancellation tokens and determine if cancellation criteria have been met. Note: it's
 * never enough to just implement an instance of [CancellationToken], you always need to
 * also add support in some [EventLoopStrategy][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.EventLoopStrategy].
 * 
 * ## Provided Implementations
 * 
 * - [HappyPathDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.HappyPathDeadline] - Timeout for normal execution
 * - [RollbackPathDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.RollbackPathDeadline] - Timeout for rollback execution  
 * - [AbsoluteDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.AbsoluteDeadline] - Overall saga timeout
 * 
...
abstract class CancellationToken<SELF : CancellationToken<SELF>>(
     ...
     * 
     * When two cancellation tokens of the same type are combined, they are merged
     * using the [and] method. This enables "most restrictive wins" behavior for
     * deadlines and similar criteria.
     * 
     ...
     * 
     * Implementations should decide how to merge two tokens, typically using
     * "most restrictive wins" logic. For deadlines, this means choosing the
     * earlier deadline. For quotas, this means choosing the lower limit.
     * 
