 ...
 * ## Why This Exists
 * 
 * Standard Jackson serialization would eagerly deserialize all context elements when reading
 * from JSON, breaking the lazy deserialization performance optimization that's central to
 * [CooperationContext]. This customizer ensures that:
 * 
 * 1. **All context data remains as JSON strings** until explicitly accessed
 * 2. **Unknown context elements from other services** are preserved without deserialization attempts
 ...
 * 
 * **Output Format**: Context is serialized as a JSON object where keys are element names
 * and values are the serialized element data:
 * ```json
 * {
 ...
 * 
 * **Input Processing**: All JSON values are read as strings and stored in [CooperationContextMap.serializedMap]
 * without any deserialization attempts. In effect, only a single level of the JSON is deserialized.
 * This preserves unknown elements and enables lazy loading.
 * 
 * **The [CooperationContextDeserializer.readAsString] Method**: This method manually reconstructs
 * JSON strings from Jackson's token stream, handling all JSON constructs (objects, arrays, primitives)
 * to preserve the exact JSON representation for later lazy deserialization.
 * 
 * ## Integration Points
 * 
 * - **Quarkus Integration**: Registered as [ObjectMapperCustomizer] for automatic setup
 * - **Serializer Caching**: The [oldSerializers] map stores the original Jackson serializers
 *   before our customization is applied. This is the simplest way to delegate back to whatever
 *   the "standard" Jackson serialization behavior would be for each specific type
 * - **ObjectMapper Injection**: Passes ObjectMapper to [CooperationContextMap] for lazy deserialization
 */
@Singleton
class CooperationContextJacksonCustomizer : ObjectMapperCustomizer {
     ...
     * 
     * This deserializer is the key to the lazy deserialization strategy - it preserves
     * all context data as JSON strings without attempting to deserialize to typed objects.
     */
    private class CooperationContextDeserializer(val objectMapper: ObjectMapper) :
         ...
         * 
         * This manually walks through Jackson's token stream and reconstructs the
         * exact JSON representation, handling all JSON constructs (objects, arrays,
         * primitives, nulls).
         */
