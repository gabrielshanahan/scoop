 ...
 * fragments that determine when a saga is ready to resume execution, and can be used to build
 * a wide array of features, including timeouts, cancellations, sleeping, and more.
 *
 ...
 * expected handlers have reacted to all emissions in the previous step. Since this is not at
 * all a trivial problem, Scoop, as a POC, only provides a dummy solution that is built on
 ...
 * which sagas are ready to execute. See
 ...
 * ## Give Up Mechanism
 * 
 * Giving up is implemented by picking up the saga for execution (due to the [giveUpOnHappyPath]/[giveUpOnRollbackPath]
 * SQL fragments saying so), which then fails immediately due to the [giveUpIfNecessary][io.github.gabrielshanahan.scoop.blocking.coroutine.continuation.BaseCooperationContinuation.giveUpIfNecessary]
 * checks in [BaseCooperationContinuation.resumeCoroutine][io.github.gabrielshanahan.scoop.blocking.coroutine.continuation.BaseCooperationContinuation.resumeCoroutine].
 */
interface EventLoopStrategy {
     ...
     * have been emitted but not yet marked as "seen" by their handlers. See
     ...
     *
     * @param candidateSeen The name of the CTE containing the SEEN event being considered for resumption
     * @param emittedInLatestStep The name of the CTE containing messages emitted in the current step  
     ...
     * @param rollbacksEmittedInLatestStep The name of the CTE containing rollback events in current step
 ...
 * - [Cancellation requests][io.github.gabrielshanahan.scoop.blocking.coroutine.structuredcooperation.StructuredCooperationCapabilities.cancel]
 * (not to be confused with [CancellationTokens][io.github.gabrielshanahan.scoop.shared.coroutine.context.CancellationToken])
 * - Various deadlines (which are special cases of [CancellationTokens][io.github.gabrielshanahan.scoop.shared.coroutine.context.CancellationToken])
 * - A pedagogical implementation of `start`
 *
 * See [HappyPathDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.HappyPathDeadline],
 * [RollbackPathDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.RollbackPathDeadline]
 * and [AbsoluteDeadline][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.deadline.AbsoluteDeadline]
