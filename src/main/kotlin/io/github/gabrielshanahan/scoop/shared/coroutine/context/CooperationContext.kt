 ...
 * 
 * This design enables cooperation across services with different codebases - each service can
 * add its own context elements while preserving unknown elements from other services.
 * 
 ...
 * for timeouts
 * - **Sleep**: See [SleepUntil][io.github.gabrielshanahan.scoop.blocking.coroutine.builder.SleepUntil]
 * 
 * ## Context Inheritance
 * 
 ...
 *
 * For more information, see https://developer.porn/posts/implementing-structured-cooperation/#cooperationcontext
 */
interface CooperationContext {
     ...
     * 
     * When both contexts contain elements with the same key, the combining behavior depends
     * on the implementation. The default behavior keeps the element from [context], but
     * elements can override [plus] to implement custom combining logic.
     * 
     * ## Custom Combining Examples
     * 
     * In [CooperationContextMap] (the most common implementation), when combining contexts
     * that both contain the same key, this method calls `element1.plus(element2)`, allowing
     * elements to define custom combining behavior:
     * 
     ...
     * The key type reflects whether this service knows about the context element:
     ...
     * 
     * This distinction enables distributed cooperation - when context flows between services,
     * each service can work with its own known elements while preserving unknown elements
     * from other services unchanged.
     * 
     ...
     * 
     * When accessed, this key type wraps the raw JSON string in an [OpaqueElement]
     * without parsing it. This enables distributed cooperation across heterogeneous services:
     ...
     * 
     * When accessed, this key type uses Jackson to deserialize the JSON string into
     * a typed Kotlin object that's defined in this service's codebase. This enables
     * type-safe access to context elements while supporting distributed cooperation.
     * 
     * Implement this abstract class to create keys for your service's context elements.
     * The class uses reflection to determine the element type for deserialization.
     * 
     * Example:
     * ```kotlin
     ...
     * of information like deadlines, cancellation tokens, or other custom data.
     */
     ...
     * 
     * This element type stores raw JSON from context elements that this service
     * doesn't recognize, enabling distributed cooperation across heterogeneous services.
     * The JSON remains unparsed, allowing it to be passed through unchanged to
     * other services that may understand it.
     */
     ...
     * 
     * Extend this class to create context elements that this service understands and
     * can work with in a type-safe manner. The framework handles serialization and
     * lazy deserialization automatically, enabling the element to flow through
     * distributed cooperation hierarchies.
     * 
