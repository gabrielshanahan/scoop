 ...
 * 
 * This cancellation token causes sagas to be canceled if they exceed the specified
 * deadline during ANY phase of execution - whether normal execution or rollback.
 * It provides an overall time limit for the entire saga lifecycle.
 * 
 ...
 * 
 * via [absoluteDeadlineMissed][io.github.gabrielshanahan.scoop.shared.coroutine.eventloop.strategy.absoluteDeadlineMissed]
 ...
 * 
 * A saga will be cancelled when ANY applicable deadline is exceeded. For example,
 * during normal execution, both HappyPathDeadline and AbsoluteDeadline are checked.
 * During rollback, both RollbackPathDeadline and AbsoluteDeadline are checked.
 * 
 ...
 * ## Trace Information
 * 
 * The `trace` field maintains a complete history of all absolute deadlines that were
 * combined to produce the current deadline. When a saga times out, trace reveals which
 * component/layer originally set the winning (most restrictive) deadline.
 *
...
data class AbsoluteDeadline(
     ...
     * 
     * The combination preserves the source information from the earlier deadline
     * and maintains a complete trace of all deadlines that contributed to the final result.
     * 
 ...
 * 
 * This is the most common way to create absolute deadlines, specifying the total
 * time allowed for the entire saga lifecycle (both normal and rollback execution).
 * 
 ...
 * 
 * Used when sagas should never be cancelled due to overall time limits,
 * typically for critical operations that must complete regardless of time.
 * 
