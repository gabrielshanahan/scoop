 ...
 * 2. **SuspendedAfterStep(<fist step name>)**: Saga completed a step and is waiting for child handlers
 * 3. **SuspendedAfterStep** with the next step name, and so on
 *
 ...
 * - **(SuspendedAfterStep, SuccessfullyRolledBackLastStep)**: Continue rolling back previous steps
 * - **(SuspendedAfterStep, ChildrenFailed...)**: Handle child failures and potentially start rollback
 *
...
sealed interface SuspensionState {
     ...
     */
    data class SuspendedAfterStep(val stepName: String) : SuspensionState
}
